...
def select_processing_mode(llm):
    """Prompt user to choose processing mode, with exit option."""
    while True:
        if llm in ("claude", "gpt", "chatgpt"):
            ...
            print("\n--- Select processing mode ---")
            print("1. Normal mode: read line by line")
            print("2. Group mode (send messages in batches to the LLM)")
            print("0. Exit script")
            read_mode = input("Choose an option: ").strip()
            if read_mode in ("1", "2"):
                return read_mode
...
def obtener_categorizacion_llm(prompt, paper, llm, strategy_folder):
            ...
            for mode in modes:
                ...
                if llm == "gemini":
                    ...
                    for idx, message, game in tqdm(
                        ...
                        try:
                            ans = call_llm_for_message(prompt, message, temp, llm, mode, game)
                            parsed = parse_llm_dict(ans)
                            parsed["original_message"] = message
                            parsed["row_id"] = idx
                            rows_buffer.append(parsed)
    ...
    # ============== GEMINI GROUP MODE =========================
    # ==========================================================
    elif read_mode == "2" and llm == "gemini":
        ...
        for temp in temps:
            for mode in modes:
                ...
                output_path = os.path.join(
                    RESULTS_PATH, "gemini", PAPER_PATHS[int(paper)], strategy_folder, out_file
                )
                os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    # ==========================================================
        ...
        print(f"⚠ Unsupported mode/LLM combination: mode={read_mode}, llm={llm}.")
        return
